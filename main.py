...
def raise_exception(
    ...
    if head is None:
        raise HTTP_Exception(st_code, message)
    else:
        raise HTTP_Exception(st_code, message, head)
...
async def login(login_data: auth.OAuth2PasswordRequestForm = auth.Depends()):
    ...
    if user is None:
        raise_exception(status.HTTP_401_UNAUTHORIZED, 'Email does not exist', {
            "Authorization": "Bearer"})
    elif not auth.verify_password(login_data.password, user.user_password):
        raise_exception(status.HTTP_401_UNAUTHORIZED, 'Bad password', {
            "Authorization": "Bearer"})
    else:
...
# A endpoint to test to authentication token for the endpoints that required

@app.get("/test/")
async def read_users_me(current_user=auth.Depends(auth.get_current_active_user)):
...
async def create_new_lobby(lobby_data: md.LobbyIn, user_id: int = auth.Depends(auth.get_current_active_user)) -> int:
    ...
    # Change current_user for "user_id"
...
async def join_lobby(lobby_id: int, user_id: int = auth.Depends(auth.get_current_active_user)):
...
async def leave_lobby(lobby_id: int, user_id: int = auth.Depends(auth.get_current_active_user)):
    is_present = dbf.is_user_in_lobby(user_id, lobby_id)
    if not is_present:
        raise_exception(status.HTTP_409_CONFLICT,
...
async def start_game(lobby_id: int, user_id: int = auth.Depends(auth.get_current_active_user)):  # Final, its ok
...
async def select_director(player_number: int, game_id: int, user_id: int = auth.Depends(auth.get_current_active_user)) -> int:  # Espera devolver un Int
    ...
    if not player_is_alive:
        raise_exception(
            status.HTTP_412_PRECONDITION_FAILED,
            (f"Player {player_nick} can't be selected as director, because is mateando con sirius Black"))
    # Check 2
    can_player_be_director = dbf.can_player_be_director(player_number, game_id)
    if can_player_be_director:
        raise_exception(
            status.HTTP_412_PRECONDITION_FAILED,
            (f"Player {player_nick} can't be selected as director, has been selected as minister or director in the last turn"))
    # Selected player as director
...
async def post_proclamation(is_phoenix_procl: bool, game_id: int, user_id: int = auth.Depends(auth.get_current_active_user)) -> int:
...
async def test_endpoint(player_id: int, user_id: int = auth.Depends(auth.get_current_active_user)):
